...
def keypoint_rcnn_inference(pred_keypoint_logits: torch.Tensor, pred_instances: List[Instances]):
    ...
    processed_results, selected_ids = heatmaps_to_keypoints(pred_keypoint_logits, bboxes_flat.detach(), num_instances_per_image)
...
def heatmaps_to_keypoints(maps: torch.Tensor, rois: torch.Tensor, num_instances_per_image) -> dict:
    ...
    for instance_idx in range(num_instances):
        ...
        kpts_offsets_pred = maps[start:end, 3*num_ori : 3*num_ori + 2*4*num_ori] # shape 127, 72, 56, 56
        ...
        for i in range(num_instances_this_image):
            ...
            roi_hmap = F.interpolate(heatmap_pred[[i]], size=outsize, mode="bicubic", align_corners=False)
            roi_heatmaps.append(roi_hmap[0]) 
            num_elements.append(roi_hmap.numel())
            
        selected_centers = top_centers(roi_heatmaps, num_elements) # 100x4 (roi_num, ori_num, x, y)
        ...
        for idx in range(len(selected_centers)):
            ...
            y = (torch.tensor(y_roi).float() + 0.5) * height_corrections[start+roi_num]

            x = x + offset_x[start+roi_num]
            y = y + offset_y[start+roi_num]
            
            x_heat, y_heat, is_inside = mapper((x,y), rois[start+roi_num],
                                                H)
            ...
            if is_inside:
                ...
                kpts_offset = kpts_offsets_pred[roi_num, ori_num*8: (ori_num+1)*8, x_heat, y_heat]
                center_reg = center_reg_pred[roi_num, :, x_heat, y_heat]
                ret_list[instance_idx].append({"pixel_loc": (x,y), "roi_num": roi_num, "roi_box": rois[start+roi_num],"width": width,
                            "scale": scale, "kpts_offset": kpts_offset,
...
def top_centers(hmap, num_elements, num=NUM_TOPS):
    ...
    for i in range(num):
        ...
        ret.append((instance_idx, *np.unravel_index(idxx.numpy(), hmap[instance_idx].shape)))  
