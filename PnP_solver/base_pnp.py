...
def cam_pose(cam_int, points2d, points3d):
    ...
    retval, rvecs, tvecs, reprojectionError = cv2.solvePnPGeneric(points3d, points2d, 
                                                        cam_int, distCoeffs = np.array([]))
    return retval, rvecs, tvecs, reprojectionError

def get_grasp_pose(kpts_2d, grasp_width, cam_ext, cam_int):
    local_3d = np.array([[0, 0, grasp_width/2],
