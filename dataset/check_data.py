...
def main(scene_id = 0):
    ...
    grasp_poses = np.array([item for sublist in grasp_poses for item in sublist])
    
    grasp_collision = data["grasp_collision"]
    grasp_collision = np.array([item for sublist in grasp_collision for item in sublist])
